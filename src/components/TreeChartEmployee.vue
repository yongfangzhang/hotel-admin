  ...
  <table v-if="treeData.name" class="tree-chart">
    <tr>
      <td :colspan="Array.isArray(treeData.children) ? treeData.children.length * 2 : 1"
          :class="{
            parentLevel: Array.isArray(treeData.children) && treeData.children.length, extend: Array.isArray(treeData.children) && treeData.children.length && treeData.extend,
            'pb-0': !treeData.children || !treeData.children.length
          }">
        <div class="node"
             @click="$emit('click-node', treeData)">
          <div class="p-2">
            <span>{{ treeData.name }}</span>
            <span v-if="treeData.employeeArr.length">
              {{ treeData.employeeArr.length }}人
            </span>
          </div>
        </div>
    ...
    <tr v-if="Array.isArray(treeData.children) && treeData.children.length && treeData.extend">
      <td v-for="(children, index) in treeData.children"
          :key="index"
          colspan="2"
          class="childLevel pb-0">
        <tree-chart-employee :json="children"
...
.tree-chart{
  ...
  .node{ min-width: 120px; position: relative; display: inline-block; margin: 0 1rem; box-sizing: border-box; text-align: center; border:2px solid #ccc;cursor: pointer;}
  /* 横板 */
